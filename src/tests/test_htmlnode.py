...
class TestHTMLNode(unittest.TestCase):
    def test_init(self):
        node = HTMLNode("tag", "value", [], {"test": "test"})
        node2 = HTMLNode()
    ...
    def test_props(self):
        ...
        for key in props.keys():
            self.assertTrue(
                key in html, "If false, the prop key was not included the html"
            )
            self.assertTrue(
                props[key] in html, "If false, the prop was not included in the html"
            )

    def test_repr(self):
        values = ["tag", "content", ["children"], {"prop": "prop"}]
        ...
        self.assertEqual(
            f"HTMLNode({values[0]}, {values[1]}, {values[2]}, {values[3]})", repr
        )
